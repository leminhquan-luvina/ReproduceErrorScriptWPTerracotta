...
def waitList = [ "org.wiperdog.directorywatcher": false]

/**
 * The body of bootup work
 */
def doBootStep() {
println "DO BOOT STEP"
try{
		...
		// println ctx.getBundle(45).class.getClassLoader().loadClass("org.terracotta.quartz.wrappers.JobWrapper")
		// println ctx.getBundle(45).loadClass("org.terracotta.quartz.wrappers.JobWrapper")

        URL [] scriptpath123 = [new File(homedir + "/" + "lib/groovy/libs.common").toURL(),new File(homedir + "/" + "lib/groovy/libs.target").toURL()]
		
		RootLoader rootloader = new RootLoader(scriptpath123, Thread.currentThread().getContextClassLoader())
		rootloader.addURL(new File(homedir + "/" + "lib/java/bundle/quartz-2.2.0.jar").toURL())
		rootloader.addURL(new File(homedir + "/" + "lib/java/bundle/org.wiperdog.scriptsupport.groovyrunner-0.2.0.jar").toURL())
		rootloader.addURL(new File(homedir + "/" + "lib/java/bundle.wrap/terracotta-toolkit-runtime-ee-4.1.0.jar").toURL())
		
		// Thread.currentThread().setContextClassLoader(rootloader)
		
		// println rootloader.loadClass("org.terracotta.quartz.wrappers.JobWrapper", true)
		// println rootloader.loadClass("Terracotta_Prototype", true)
		// println rootloader.loadClass("org.terracotta.quartz.TerracottaJobStore", true)
		// rootloader.loadClass("org.quartz.JobDetail", true)
...
//        def jettyLoader = new JettyLoader(ctx)
		...
		// --------------TERRACOTTA--------------
		def terracottaPrototype = rootloader.loadClass("Terracotta_Prototype").newInstance(rootloader)
		// --------------/TERRACOTTA--------------
